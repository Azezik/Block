export function createInvestmentSettingsRow({
  investment,
  index,
  onNameInput,
  onPercentInput,
  onExistingAmountInput,
  onOverflowRateInput
}) {
  ...
  row.innerHTML = `
    ...
    <input class="field" data-role="overflow-rate" data-index="${index}" type="number" min="0" step="0.1" value="${Math.max(0, Number(investment.overflowRatePerMinute ?? 1))}" placeholder="Overflow rate/min" aria-label="Overflow rate per minute for ${investment.name || `investment ${index + 1}`}" >
  `;
  ...
  row.querySelector('[data-role="overflow-rate"]').addEventListener('input', onOverflowRateInput);
